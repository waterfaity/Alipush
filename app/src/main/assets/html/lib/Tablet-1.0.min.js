...
(function (window) {
    ...
    Tablet.prototype._init = function (_container, config) {
        ...
        this.point = {x: 0, y: 0};
        this.points = {x: [], y: [], left: 0, right: 0, top: 0, bottom: 0};
        this.degree = 0;
                ...
                if ($self.hasClass("color-color")) {
                    $self.colpick({
                        ...
                        onSubmit: function (hsb, hex, rgb, el, bySetColor) {
                            var color = "#" + hex;
                            $(el).css({"border-color": color}).colpickHide();
                            el.selectedColor = color;
                            that.color = color;
                            that.setColor(color)
                        }
                    });
                    $self.css({"border-color": that.config.defaultColor})
                } else {
                    if ($self.hasClass("bg-color")) {
                        ...
                        $self.css({"border-color": that.config.defaultBackgroundColor})
                    }
                }
            })
        }
    ...
    Tablet.prototype._ctxInit = function () {
        var that = this, pait = function (singal) {
                switch (singal) {
                    case 1:
                        that.ctx.beginPath();
            ...
            }, clientRect = that.clientRect = that.canvas.getBoundingClientRect(), pressed = this.pressed = false,
        ...
        var start = create(1), move = create(2), animationMove = window.requestAnimationFrame ? function (e) {
    ...
    Tablet.prototype.setCanvasWH = function (width, height) {
        var that = this, $tabletBtns = this.tablet.find(".tablet-btns"),
        ...
        if (!width || !height) {
            var _w = 0, _h = 0, $canvasParent = this.tablet;
            if (!that.isMobile) {
                ...
                _h = winH;
                that.tablet.children(".-tablet-container").css({width: _w, height: _h})
            }
            this.width = _w - bl - br;
            this.height = _h - $tabletBtns.outerHeight() - bt - bb
        } else {
            this.tablet.children(".-tablet-container").css({width: width, height: height});
    ...
    Tablet.prototype.getBase64 = function (type) {
        var that = this, position = Tablet.getMax(this.points.x, this.points.y);
        if (!type) {
            type = "image/png"
        } else {
    ...
    Tablet.prototype.getBlob = function (type) {
        var that = this, base64Img = this.getBase64(type), arr = base64Img.split(","),
            mime = arr[0].match(/:(.*?);/)[1], bStr = atob(arr[1]), len = bStr.length, u8arr = new Uint8Array(len);
        while (len--) {
    ...
    Tablet.prototype.rotate = function (degree) {
        var minDeg = -90, maxDeg = 180, angle = 0, that = this, translateLen = 0, winW = $(window).width(),
            winH = $(window).height(), w = winW, h = winH, newCanvas = $("<canvas>");
        degree = ~~degree;
        if (degree < minDeg) {
            degree = minDeg
        }
    ...
    Tablet.getMax = function (xPoints, yPoints) {
        var obj = {left: 0, right: 0, top: 0, bottom: 0};
