/**
 * @author azrael
 * @date 2013-2-6, thanks to azrael
 * 
 */
package com.alibaba.cloudpushdemo.jsb;
...
import com.alibaba.cloudpushdemo.JSONUtil;
...
public class JavascriptBridge {
    ...
    class JSObject{
        ...
        public void setResult(long serial, String jsonResult){
            ...
            if(command.callback != null && jsonResult != null && !jsonResult.equals("")){
                try {
         ...
         * @param params
         * @return java方法执行的返回值
         * update x 2014-12-20有些手机拿不到返回值的BUG
         */
        @JavascriptInterface
        public String require(String cmd, String params){
            Function function = javaMethodMap.get(cmd);
            if(function != null){
                try {
                    JSONObject json = new JSONObject(params);
//                    System.out.println("function.execute(json).toString():"+function.execute(json).toString());
                    return  function.execute(json).toString();
                } catch (JSONException e) {
                    e.printStackTrace();
                }
            }
    ...
    public JavascriptBridge(WebView mWebView) {
        ...
        mWebView.addJavascriptInterface(new JSObject(), API_NAMESPACE);
        // add x alert 页面直接用来提示 安卓的信息框
        final Context context = mWebView.getContext();
        javaMethodMap.put("alert", new JavascriptBridge.Function() {
			@Override
